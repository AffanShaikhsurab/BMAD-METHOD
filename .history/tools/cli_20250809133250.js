...
const LearningsStore = require('./lib/learnings');
...
program
  .command('learnings:validate')
  .description('Validate and initialize the .bmad/learnings manifest and category folders')
  .action(async () => {
    try {
      const store = new LearningsStore({ rootDir: process.cwd() });
      const info = await store.validateOrInit();
      console.log('Learnings store is ready:');
      console.log(`  Base: ${info.baseDir}`);
      console.log(`  Manifest: ${info.manifestPath}`);
      console.log('  Categories:');
      Object.entries(info.categories).forEach(([k, v]) => console.log(`    - ${k}: ${v}`));
    } catch (error) {
      console.error('Learnings validation failed:', error.message);
      process.exit(1);
    }
  });

program
  .command('learnings:index:rebuild')
  .description('Rebuild the per-category and global learnings indices')
  .option('-c, --category <name>', 'Rebuild only a specific category')
  .action(async (options) => {
    try {
      const store = new LearningsStore({ rootDir: process.cwd() });
      await store.validateOrInit();
      const res = await store.rebuildIndex(options.category);
      console.log(`Rebuilt index for: ${res.rebuilt}`);
    } catch (error) {
...
program
  .command('learnings:index:list')
  .description('List summaries from the learnings index (global or per category)')
  .option('-c, --category <name>', 'List only a specific category')
  .action(async (options) => {
    try {
      const store = new LearningsStore({ rootDir: process.cwd() });
      await store.validateOrInit();
      const entries = await store.listIndex(options.category);
      if (!entries.length) {
        console.log('No learnings found.');
        return;
      }
      entries.forEach((e) => {
        console.log(`- [${e.category || options.category || 'unknown'}] ${e.title} -> ${e.file}`);
      });
    } catch (error) {
